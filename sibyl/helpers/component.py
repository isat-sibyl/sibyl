...
class Component:
	...
	def get_imported_requirements(self):
		...
		for imported_requirement in imported_requirements:
			...
			if imported_requirement["type"] == "script":
				...
				raise ValueError("Unknown requirement type: " + imported_requirement["type"])
			result.append(Requirement(imported_requirement["name"], req_type, imported_requirement["src"].replace("\\", "/")))
		return result
	...
	def run_python_phase(self, context : dict, phase : str):
		...
		for python in self.pythons:
			...
			if python.has_attr(phase) or (not python.has_attr("cleanup") and phase == "default"):
